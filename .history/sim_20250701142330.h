  ...
  while (!PQEmpty(&(s->pq)))
  {
    ...
    *e = PQDelete(&(s->pq));
    ...
    if (GetEventType(e) == arrival)
    {
      ...
      printf("Time: %2d\tarrival of %s customer %d\n", GetTime(e), (GetCustomerType(e) == Vip) ? "VIP" : "regular", GetCustomerID(e));

      /* generate departure event for current customer */
      servicetime = Get_ServiceTime(s);
      iv = GetCustomerType(e);
      tellerID = NextAvailableTeller(s, iv); // Assign teller based on VIP status

      /* If teller is free, update finishService to current time */
      ...
      /* Log VIP queue-cutting if applicable */
      if (iv == Vip && tellerID != 1 && s->tstat[tellerID].finishService > GetTime(e))
      {
        printf("\tVIP customer %d cuts in line at regular teller %d\n", GetCustomerID(e), tellerID);
      }
    }
    /* handle a departure event */
    else
    {
      printf("Time: %2d\tdeparture of %s customer %d\n", GetTime(e), (GetCustomerType(e) == Vip) ? "VIP" : "regular", GetCustomerID(e));
      printf("\tTeller %d\tWait %d\tService %d\n", GetTellerID(e), GetWaitTime(e), GetServiceTime(e));
      tellerID = GetTellerID(e);
      /* If nobody is waiting for teller, mark teller free */
