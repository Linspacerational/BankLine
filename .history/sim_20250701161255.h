...
#include "apqueue.h"
isVip ivs[10] = {notVip, notVip, Vip, Vip, Vip, notVip, Vip, notVip, notVip, Vip};
...
void InitSimulation(Simulation *s)
{
  ...
  InitEvent(firstevent, 0, arrival, 1, 0, 0, 0, ivs[0]);
  InitPQueue(&(s->pq));
    ...
    *e = PQDelete(&(s->pq));
    ...
    if (GetEventType(e) == arrival)
    {
      ...
      if (nexttime <= s->simulationLength)
      {
        /* 生成下一个客户的到达事件 */
        s->nextCustomer++;
        InitEvent(newevent, nexttime, arrival, s->nextCustomer, 0, 0, 0, ivs[ivindex++]);
      ...
      if (s->tstat[tellerID].finishService + servicetime > s->simulationLength)
      {
        /* 如果服务时间超过模拟长度，则不处理该客户 */
        printf("\t出纳员 %d 完成服务时间为 %d，无法处理客户 %d，服务时间超过模拟长度\n", tellerID, s->tstat[tellerID].finishService, GetCustomerID(e));
        continue;
      }
      /* 计算等待时间 */
