...
int NextAvailableTeller(Simulation *s, isVip iv, int currentTime)
{
  ...
  if (iv == Vip)
  {
    ...
    // 2. If no idle tellers, VIP customer prioritizes Teller #1 for queuing
    // This is the VIP-specific rule to prioritize the VIP teller.
    return 1; // Assign to Teller 1 for queuing
  }
  else // notVip
  {
    ...
    // This scenario assumes there's at least one regular teller if numTellers > 1.
    if (bestTellerID == -1)
    {
      // If no suitable regular teller is found, and numTellers is configured
      // to have regular tellers (i.e. numTellers >= 2), this indicates a problem.
      // For robustness, returning teller 2 as a default if it exists.
      if (s->numTellers >= 2)
...
void RunSimulation(Simulation *s)
{
  ...
  // This value is used in PrintSimulationResults for teller work percentage.
  // We need the maximum time an event *occurred* or *finished*.
  // It should be the time of the last processed departure event if it's greater than initial simulationLength.
  if (GetTime(e) > s->simulationLength)
  {
