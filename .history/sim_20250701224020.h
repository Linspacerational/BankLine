...
void InitSimulation(Simulation *s)
{
  ...
  // 预定义的初始客户 VIP 状态
  isVip temp[10] = {Vip, notVip, Vip, Vip, Vip, notVip, Vip, notVip, notVip, Vip};
  for (i = 0; i < 10; i++)
  {
    s->ivs[i] = temp[i];
  }
  ...
  while (!PQEmpty(&(s->pq)))
  {
    *e = PQDelete(&(s->pq)); // 从优先队列中获取下一个事件
    ...
    if (GetEventType(e) == arrival)
    {
      ...
      else // 出纳员忙碌或有队列，客户需要排队
      {
        // 处理 VIP 抢占逻辑 (这部分你在上次修改中已经有了)
        if (iv == Vip && tellerID != 1 && !IsTellerQueueEmpty(&s->tstat[tellerID]))
        {
          Event nextInQueue = PeekCustomer(&s->tstat[tellerID]);
          if (GetCustomerType(&nextInQueue) != Vip)
          {
            // 抢占逻辑：将 VIP 放在队头，被抢占的客户放回队尾
            Event preemptedCustomer = DequeueCustomer(&s->tstat[tellerID]);
            // 重新初始化被抢占的客户事件，使其“到达时间”变为当前VIP到达时间，以延长其等待计算
            InitEvent(&preemptedCustomer, GetTime(e), GetEventType(&preemptedCustomer),
                      GetCustomerID(&preemptedCustomer), GetTellerID(&preemptedCustomer),
                      GetWaitTime(&preemptedCustomer), GetServiceTime(&preemptedCustomer),
                      GetCustomerType(&preemptedCustomer));

            EnqueueCustomer(&s->tstat[tellerID], *e);                // VIP 入队
            EnqueueCustomer(&s->tstat[tellerID], preemptedCustomer); // 被抢占客户入队
            printf("\tVIP客户 %d 在普通出纳员 %d 插队\n", GetCustomerID(e), tellerID);
          }
          else
          {
            // VIP 在另一个 VIP 或普通客户后面排队
    ...
    else // GetEventType(e) == departure
    {
      ...
      if (!IsTellerQueueEmpty(&s->tstat[tellerID]))
      {
        ...
        int serviceTimeForNext = Get_ServiceTime(s); // 获取下一个客户的服务时间
        // 下一个客户的等待时间是从其到达时间到出纳员空闲（当前离开时间）的时间
        // 这里需要更精确的等待时间计算。
        // 等待时间 = 出纳员空闲时间 - 客户实际到达时间
        // 对于队列中的客户，他们的“排队开始时间”是他们被 EnqueueCustomer 时的 GetTime(e)
        // 但这里 GetTime(e) 是前一个客户的 departure 时间。
        // 应该用 nextCustomerInLine 的原始 arrivalTime 来计算等待时间。

        // 修正等待时间计算：
        // 等待时间是：(出纳员变为可用时间) - (客户到达时间)
        // 如果客户到达时出纳员已空闲，则等待时间为 0。
        // 否则，等待时间是 (前一个客户离开时间 或 出纳员预计空闲时间) - 客户的真实到达时间。
        // 简化起见，这里假设 nextCustomerInLine.time 已经包含了其排队开始时间。
        ...
        // 计算下一个客户的离开时间
        int nextCustomerDepartureTime = GetTime(e) + serviceTimeForNext; // 从当前出纳员空闲时间开始服务
        s->tstat[tellerID].finishService = nextCustomerDepartureTime;    // 更新出纳员的空闲时间

        InitEvent(newevent, nextCustomerDepartureTime, // 离开时间
                  departure, GetCustomerID(&nextCustomerInLine), tellerID,
                  waitTimeForNext, serviceTimeForNext, GetCustomerType(&nextCustomerInLine));
        PQInsert(&(s->pq), *newevent); // 将新离开事件插入到主优先队列
