...
void InitSimulation(Simulation *s)
{
  ...
  printf("输入出纳员数量：");
  scanf("%d%d", &s->numTellers); // Assuming numTellers is now read as a single integer
  printf("输入到达时间范围（分钟）：");
...
int NextAvailableTeller(Simulation *s, isVip iv, int currentTime)
{
  ...
  else // notVip
  {
    ...
    if (bestTellerID == -1)
    {
      ...
      // For robustness, if only Teller 1 exists or no regular tellers are selectable,
      // then assign to Teller 1 (which would then be the only option).
  ...
  while (!PQEmpty(&(s->pq)))
  {
    *e = PQDelete(&(s->pq)); // Get the next event from the priority queue
    ...
    if (GetEventType(e) == arrival)
    {
      ...
      {
        // VIP customer might preempt into a regular teller's queue (InsertVipIntoQueue handles placement)
        // Or a customer (VIP or non-VIP) simply queues.
        // Note: The logic for 'tellerID != 1' for VIP preemption is within InsertVipIntoQueue
        // or implied by its placement logic in a general queue.
        // Here, if it's a VIP assigned to a non-VIP teller or any customer to a busy teller, they queue.
        if (iv == Vip && tellerID != 1)
        { // VIP assigned to a regular teller
          InsertVipIntoQueue(&s->tstat[tellerID], *e);
          printf("\tVIP客户 %d 在出纳员 %d 排队 (插队)\n", GetCustomerID(e), tellerID);
        }
        else
        {
